...
data.exp1 <- 
  read_csv(here::here("data/TOJ_exp1.csv")) %>% # load data
  ...
  summarize(
    num.horiz1st = length(which(response == 2)), # number of "horizontal first" responses
...
ACC.plot <- 
  ggplot(summary.data.exp1, aes(SOA, prop.horizfirst, group = cue, color = cue, fill = cue)) + # basic graph
  ...
  geom_hline(
    yintercept = seq(0, 1, .25), # reference lines
...
.center[
  ...
  # load results model
  readRDS(here::here("results/pseudoBMA.rds")) %>%
...
# load results model
model.mains <- readRDS(here::here("results/model_mains.rds"))
...
summary.data.exp1.est <- 
  data.exp1 %>%
  mutate(
    num.horizfirst.est = predict(model.mains)[, 1], # add estimated accuracy to original dataframe
...
ACC.plot.obs.est <- 
  summary.data.exp1.est %>% # summary table
...
samples.model.mains <- 
  as.data.frame(posterior_samples(model.mains, "b")) # posterior samples of all betas (on log scale)
